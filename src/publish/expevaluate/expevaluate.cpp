...
const CExpEvaluate::TokenType CExpEvaluate::GetToken (const bool ignoreSign)
{
	...
	if ( cFirstCharacter == '0' && cNextCharacter == 'x' )
	{	// 16 进制数支持
		// by dummyz@126.com
		char c;
		unsigned int t = 0;
...
const double CExpEvaluate::Evaluate (const std::string & program, Function_EvaluateCallback pfn, long userdata)  // get result
{
	// do same stuff constructor did
	m_program  = program;
	m_pWord    = m_program.c_str ();
	m_type     = TOKEN_NONE;
	return Evaluate ( pfn, userdata );
}

const double CExpEvaluate::Evaluate( Function_EvaluateCallback pfn, long userdata )  // get result
{
	m_pfnCallback = pfn;
	m_userdata = userdata;
	...
	case TOKEN_NAME:
		{
			...
			if (m_type == TOKEN_LHPAREN)
			{
				...
				GetToken();
				double v = 0.0;
				if( m_pfnCallback(word.c_str(), params, v, m_userdata) )
					return v;
